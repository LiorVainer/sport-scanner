...
export const PackagesGenerationParamsWithStringDatesSchema = z
    .object({
        originIATA: z
            .string()
            .min(1, { message: 'Origin Airport is required' })
        ...
        date: z
            .object({
                from: z
                    .string()
                    .regex(/^\d{4}-\d{2}-\d{2}$/)
                    .describe('Earliest departure date (ISO format: YYYY-MM-DD)'),
                to: z
                    .string()
                    .regex(/^\d{4}-\d{2}-\d{2}$/)
                    .describe('Latest return date (ISO format: YYYY-MM-DD)'),
            })
            .describe('Date range of the user’s availability for travel'),
...
export const PackagesGenerationParamsFromFreeTextSchema = z
    .object({
        originIATA: z
            .string()
            .min(1, { message: 'Origin Airport is required' })
...
export type PackagesGenerationParamsWithStringDates = z.infer<typeof PackagesGenerationParamsWithStringDatesSchema>;
